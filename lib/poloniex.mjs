...
export default class Poloniex {
  ...
  static #apiRateLimitsEndpointsSets = {
  ...
  constructor({ apiKey = '', apiSecret = '' } = {}) {
    ...
    for (const k of Object.keys(Poloniex.#apiRateLimitsEndpointsSets)) {
      Poloniex.#apiRateLimitsEndpointsSets[k] = makeRegexp(Poloniex.#apiRateLimitsEndpointsSets[k])
    }
  ...
  static #classifyApiCall(method, path) {
    const endpoint = method + path
    let clas
    for (const [setName, set] of Object.entries(Poloniex.#apiRateLimitsEndpointsSets)) {
...
// Copies all keys from src object to a destination object, except excludeKeys. It returns the destination object.
  ...
  #requestPub(method, path, params, body, getApiCallRateInfo) {
    if (getApiCallRateInfo) {
      const key = Poloniex.#classifyApiCall(method, path)
      return [key, this.apiCallRate[key], this.apiCallRateLimits[key]]
    }
  ...
  #requestAuth(method, path, params, body, getApiCallRateInfo) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const url = Poloniex.#baseUrl.rest + path
      axios({ method, url, headers, params, data })
        .then((res) => resolve (res.data))
        .catch((err) => reject(err))
    })
  }
  ...
  /**
   * Call rate for resource-intensive and non-resource-intensive private and public API calls
   *
   * @type {{riPriv: number, riPub: number, nriPub: number, nriPriv: number}}
   */
  apiCallRate = {
